...
import lib.Click;
import lib.ProjectFunctions;
import lib.TypeText;
import lib.Utility;

public class ServiceNow {
	// Code Version 1/17/23

	String Application = "ServiceNow Automation"; // For New Application Change here required
	String ExcelImportFile = "ServiceNow.xlsx"; // For New Application Change here required
	String Env = "Prod Environment"; // For New Application Change here required
	...
	public void BM() throws Exception {
		...
		Date date = new Date();
		// recorder = new ATUTestRecorder(VideoPath,Application +" - " +
						...
						if(ObjectType.equalsIgnoreCase("Browser"))
						{
							if (Action.equalsIgnoreCase("StartBrowser"))
							{
								...
								/*    								
									 ...
									 ChromeDriver(capability); driver.manage().window().maximize();
								 */
						...
						if(ObjectType.equalsIgnoreCase("Utility"))
						 {
						   if(Action.equalsIgnoreCase("Frame"))
						       {  
							 String status  = Utility.Switchframe(driver, LocatorType, TestData);
						   ...
						   if(Action.equalsIgnoreCase("CompareText"))
							...
							if (status.equalsIgnoreCase("Pass"))
								...
								{   
									...
									//***************************  Type and Scroll down select dropdown for SericeNow Only *************************** 
								...
								if(ObjectType.equalsIgnoreCase("Click"))
								{
									 if(Action.equalsIgnoreCase("Dropdown"))
								       {  
										 ...
										 String status = ProjectFunctions.ServiceNow(driver, LocatorType, LocatorName);
										  System.out.print("Move to Function Click + Dropdown");
										 
										 if (status.equalsIgnoreCase("pass")) { //
										  Utility.captureScreenshot(driver, Snappath, Description);
										  logger.log(LogStatus.PASS, Testcases, Description +
										  logger.addScreenCapture(screenshotpath));
